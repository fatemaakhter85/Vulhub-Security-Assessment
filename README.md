# 🔐 Vulhub Vulnerability Analysis & Exploitation Project

**Prepared by:** Fatema Akhter | Sabreone Quantum SybreSecurity  
**Project ID:** PENT54354  
**Version:** 1.2 | **Date:** March 03, 2025  
**Mentor:** Rafsan Anwar — Head of Dept. Network & Cybersecurity, Creative IT Institute

---

## 📌 Project Overview

This project is a comprehensive **Vulnerability Analysis and Exploitation** assessment conducted on **VulHub** virtual machines. The assessment was performed as part of the Career Advancement Programme (CAP) in Cybersecurity at Creative IT Institute.

The goal was to identify, analyze, and exploit vulnerabilities in realistic environments using industry-standard methodologies including **OWASP Testing Guide** and **Penetration Testing Execution Standard (PTES)**.

---

## 🎯 Objectives

- Identify vulnerabilities in VulHub virtual machines
- Analyze and classify vulnerabilities based on impact and risk
- Demonstrate exploitation process in a controlled environment
- Recommend countermeasures and mitigations

---

## 🖥️ Targets Tested

| # | Target | Type |
|---|--------|------|
| 01 | The Planets: Earth | Vulnerable VM (VulnHub) |
| 02 | Jangow: 1.0.1 | CTF Machine — Privilege Escalation |
| 03 | Metasploitable: 2 | vsftpd 2.3.4 Exploitation |

---

## 🧰 Methodology & Tools Used

- **Reconnaissance** — Network scanning, enumeration
- **Vulnerability Scanning** — Nessus Web App Scanner
- **Exploitation** — Metasploit, Manual techniques
- **Privilege Escalation** — Gaining root access
- **Reporting** — Documented findings with Proof of Concept (PoC)

---

## 📅 Timeline

| Week | Activity |
|------|----------|
| Week 1-2 | Vulnerability Scanning |
| Week 3-4 | Exploitation & Documentation |
| Week 5 | Report Preparation |
| Week 6 | Final Review & Delivery |

---

## 📄 Full Report

👉 [Click here to view the full PDF report](./report.pdf)

---

## 🏫 Institute Information

**Creative IT Institute**  
Mentor: Rafsan Anwar  
Phone: 01312-001312  
Email: rafsan.cit.bd@gmail.com

---

## ⚠️ Disclaimer

This project was conducted in a **controlled lab environment** for educational purposes only. All findings are documented ethically and responsibly following responsible disclosure practices.
